//controller allows for templating using ThymeLeaf
...
// private static List<BlogPost> posts = new ArrayList<>();  *this line not necessary, per Leo*
    @GetMapping("/")
...
//        posts.add(blogPost);
    ...
    public String editPostById(@PathVariable Long id, BlogPost blogPost, Model model) {
        model.addAttribute("blogPost", blogPostRepository.findById(id).orElse(new BlogPost()));
        return "blogpost/edit";
    }

    @PostMapping("/blogposts/update/{id}")
    public String updateExistingPost(@PathVariable Long id, BlogPost blogPost, Model model){
        var actualPost = blogPostRepository.findById(id).orElse(new BlogPost());
